class MeetingDatesController < PrivateController
  ...
  def index
    ...
    #@meeting_dates = @meeting_dates.joins(:meeting).where(meetings: {user: current_user}) unless current_user.admin?
    @meeting_dates = @meeting_dates.paginate(:page => params[:page])
  end
  ...
  private
    ...
    def set_meeting_date
      @meeting_date = MeetingDate.find(params[:id] || params[:meeting_date_id])
      #@meeting_date ||= UserDate.find(params[:user_date_id]).meeting_date if params[:user_date_id]
    end
